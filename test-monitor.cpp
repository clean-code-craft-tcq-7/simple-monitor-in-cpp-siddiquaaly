...
{ //enter the current vital value of temperature, pulseRate, spo2
  ...
  EXPECT_FALSE(vitalsCheck(currentValues)) << "Temp is high, condition is critical";
}

TEST(Monitor_Temperature, TemperatureIsLow) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {90.5,80,95}
  EXPECT_FALSE(vitalsCheck(currentValues)) << "Temp is low, condition is critical";
}

TEST(Monitor_Temperature, TemperatureIsNormal) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {98.8,80,95}
  EXPECT_TRUE(vitalsCheck(currentValues)) << "Temp is Normal, All OK";
}

TEST(Monitor_PulseRate, PulseRateIsHigh) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {98,102,95}
  EXPECT_FALSE(vitalsCheck(currentValues)) << "PulseRate is high, condition is critical";
}

TEST(Monitor_PulseRate, PulseRateIsLow) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {96,59,95}
  EXPECT_FALSE(vitalsCheck(currentValues)) << "PulseRate is low, condition is critical";
}

TEST(Monitor_PulseRate, PulseRateIsNormal) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {100,80,95}
  EXPECT_TRUE(vitalsCheck(currentValues)) << "PulseRate is Normal, All OK";
}

TEST(Monitor_Sp02, Sp02IsLow) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {96,80,88}
  EXPECT_FALSE(vitalsCheck(currentValues)) << "Sp02 is low, condition is critical";
}

TEST(Monitor_Sp02, Sp02IsNormal) 
{ //enter the current vital value of temperature, pulseRate, spo2
  vector<float> currentValues = {100,80,95}
  EXPECT_TRUE(vitalsCheck(currentValues)) << "Sp02 is Normal, All OK";
}